...
app.get('/file/:id', isLoggedIn ,async (req, res) => {
...
app.get('/back', isLoggedIn , async(req,res)=>{
    let user = await User.findOne({email:req.user.email})
    res.redirect(`/indexpage/${user._id}`)
})
...
app.post('/edit', isLoggedIn ,async(req, res) => {
    let { title, description, id } = req.body;//id is of note
    console.log(req.user)
    let user = await User.findOne({ email: req.user.email  })
...
console.log(note)
 ...
 let update = await User.updateOne({email:user.email},{$pull:{
   notes:req.params.id
 }})

 let deletenote = await Note.findOneAndDelete({_id:note._id})
